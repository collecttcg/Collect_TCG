...
function compactGeneratedPostSpacing(value){
  ...
  // editable fields deliberately remain as entered so card-printing language
  // and seller wording are never changed by a template choice.
...
function defaultCarousellProductDetails(card,language="en"){
    ...
    const labels={en:["Year","Series","Game","Language","Price Terms"],ms:["Tahun","Siri","Permainan","Bahasa","Syarat Harga"],zh:["年份","系列","游戏","语言","价格条款"],ja:["年","シリーズ","ゲーム","言語","価格条件"],ko:["연도","시리즈","게임","언어","가격 조건"]}[appContext.normalizePostLanguage(language)]||["Year","Series","Game","Language","Price Terms"];
...
function buildCarousellPostText(productDetails,language="en"){
    ...
    if(selectedLanguage!=="en"){
      ...
      return appContext.compactGeneratedPostSpacing([text.carousellNoTrade,"",text.productDetails,details,"",text.caution,local[0],local[1],"",text.importantNotes,`- ${local[2]}`,`- ${local[3]}`,`- ${local[4]}`,`- ${local[1]}`,"",text.codRules,`1. ${local[5]}`,`2. ${local[6]}`,`3. ${local[7]}`,`4. ${local[8]}`,`5. ${local[9]}` ,"",local[10]].join("\n"));
    }
